...
# Need to create two tables (one for FIELD data and one for LAB data)
    def create_field_data_table(self):
            for (url, i) in zip(
                self.response_object['desktop'].keys(),
                range(0, len(self.response_object['desktop'].keys()))
            ):
                try:
                    print('Trying to insert response for url:', url)

                    #We reuse this below when selecting data from the response
                    fcp_loading = self.response_object['desktop'][url]['loadingExperience']['metrics']['FIRST_CONTENTFUL_PAINT_MS']

                    # --desktop--:
                    # Fill urls
                    self.df_field_responses.loc[i, 'requested_url'] = \
                        self.response_object['desktop'][url]['lighthouseResult']['requestedUrl']
                    self.df_field_responses.loc[i, 'final_url'] =\
                        self.response_object['desktop'][url]['lighthouseResult']['finalUrl']

                    # Fill device type
                    self.df_field_responses.loc[i, 'device_type'] = 'desktop'
